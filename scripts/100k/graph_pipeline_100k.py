"""100k multi-pattern graph pipeline.

Generates peel-chain AND smurfing synthetic transactions, merges them
with real 100k Ethereum data, and writes augmented CSVs + wallet features
to ``data/processed/100k/``.
"""
...
DATA_PROCESSED_DIR = PROJECT_ROOT / "data" / "processed" / "100k"

SOURCE_FILE = DATA_RAW_DIR / "eth_transfers_100k_2024.csv"

AUGMENTED_FILE = DATA_PROCESSED_DIR / "augmented_transactions_multipattern.csv"
FEATURES_FILE = DATA_PROCESSED_DIR / "wallet_features_multipattern.csv"
LABELS_FILE = DATA_PROCESSED_DIR / "transaction_labels_multipattern.csv"
...
def _expected_transactions_per_smurf(mule_count_weights: dict[int, float]) -> float:
    ...
    for mule_count, weight in mule_count_weights.items():
        # Each cluster produces 2*N transactions (fan-out + fan-in).
...
def build_wallet_features(transactions: pd.DataFrame) -> pd.DataFrame:
    ...
    betweenness = nx.betweenness_centrality(graph, k=100, seed=42)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to raw source CSV. Defaults to data/raw/100k/eth_transfers_100k_2024_final.csv.",
    )
    ...
    augmented_file = output_dir / "augmented_transactions_multipattern.csv"
    features_file = output_dir / "wallet_features_multipattern.csv"
    labels_file = output_dir / "transaction_labels_multipattern.csv"
    ...
    # Update existing wallets to avoid collisions between peel and smurf wallets.
    ...
    # Use a different seed offset for smurfing to avoid RNG correlation.
    ...
    def _assign_time_block(n: int) -> tuple[list[str], list[int]]:
        times, blocks = [], []
        for _ in range(n):
            base = rng.choice(valid_times)
            offset = rng.randint(0, 59)
            times.append((base + pd.Timedelta(seconds=offset)).strftime("%Y-%m-%d %H:%M:%S.000 UTC"))
            blocks.append(rng.choice(block_numbers))
        return times, blocks

    empty_cols = ["amount", "block_number", "block_time", "from", "to", "tx_hash"]
    
    if len(peel_df) > 0:
        peel_times, peel_blocks = _assign_time_block(len(peel_df))
        ...
        peel_synthetic = pd.DataFrame(columns=empty_cols)

    if len(smurf_df) > 0:
        smurf_times, smurf_blocks = _assign_time_block(len(smurf_df))
        ...
        smurf_synthetic = pd.DataFrame(columns=empty_cols)
