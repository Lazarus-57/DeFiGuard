"""100k peel-chain-only graph pipeline.

Generates peel-chain synthetic transactions, merges them with real 100k
Ethereum data, and writes augmented CSVs + wallet features to
``data/processed/100k/peel_only/``.
"""
...
DATA_PROCESSED_DIR = PROJECT_ROOT / "data" / "processed" / "100k" / "peel_only"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build augmented 100k AML dataset with peel-chain patterns only."
    )
    ...
    # ---- Combine everything ----
    combined = pd.concat([df, peel_synthetic], ignore_index=True)
