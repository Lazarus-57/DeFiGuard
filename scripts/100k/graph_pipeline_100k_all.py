...
DATA_PROCESSED_DIR = PROJECT_ROOT / "data" / "processed" / "100k" / "all_patterns"
...
def _expected_transactions_per_ring(ring_size_weights: dict[int, float]) -> float:
    total_weight = float(sum(ring_size_weights.values()))
    if total_weight <= 0:
        raise ValueError("ring_size_weights must have a positive total weight.")
    expected = 0.0
    for ring_size, weight in ring_size_weights.items():
        if ring_size < 3:
            raise ValueError("Each ring size must be >= 3.")
        expected += (weight / total_weight) * ring_size
    return expected

def _estimate_ring_count_for_target_rate(
    n_base_rows: int,
    target_positive_rate: float,
    ...
    if not (0.0 < target_positive_rate < 1.0):
        raise ValueError("target_positive_rate must be between 0 and 1.")
    expected_tx = _expected_transactions_per_ring(ring_size_weights)
    target_pos = (target_positive_rate * n_base_rows) / (1.0 - target_positive_rate)
    return max(1, int(ceil(target_pos / expected_tx)))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build augmented 100k AML dataset with peel-chain, smurfing, AND circular patterns."
    )
    ...
    parser.add_argument(
        "--circular-target-rate",
        type=float,
        default=0.03,
        help="Target circular positive rate (default: 0.03).",
    )
    parser.add_argument("--num-peel-chains", type=int, default=None)
    parser.add_argument("--num-smurf-clusters", type=int, default=None)
    parser.add_argument("--num-circular-rings", type=int, default=None)
    ...
    smurf_wallets = set(smurf_df["from"].tolist()) | set(smurf_df["to"].tolist())
    existing_wallets = existing_wallets | smurf_wallets

    # ---- Circular rings ----
    ring_size_weights = {3: 0.3, 4: 0.4, 5: 0.2, 6: 0.1}
    num_circular = args.num_circular_rings
    if num_circular is None:
        num_circular = _estimate_ring_count_for_target_rate(
            n_base_rows=len(df),
            target_positive_rate=args.circular_target_rate,
            ring_size_weights=ring_size_weights,
        )

    circular_df = generate_circular_laundering(
    ...
    def _assign_sequential_times_circular(df: pd.DataFrame) -> tuple[list[str], list[int]]:
        times, blocks = [], []
        current_time_map = {}
        for idx, row in df.iterrows():
            pid = row["pattern_id"]
    ...
    circular_times, circular_blocks = _assign_sequential_times_circular(circular_df)
    ...
    circular_synthetic = pd.DataFrame({
        "amount": circular_df["amount"],
        "block_number": circular_blocks,
        "block_time": circular_times,
        "from": circular_df["from"],
        "to": circular_df["to"],
        "tx_hash": [f"0x{rng.getrandbits(256):064x}" for _ in range(len(circular_df))],
    })

    # ---- Combine everything ----
    combined = pd.concat([df, peel_synthetic, smurf_synthetic, circular_synthetic], ignore_index=True)
    ...
    labels_circular = pd.DataFrame({
        "tx_hash": circular_synthetic["tx_hash"],
        "aml_label": 1,
        "label_note": "circular",
    })
    labels = pd.concat([labels_normal, labels_peel, labels_smurf, labels_circular], ignore_index=True)
    ...
    print()
    print(f"Circular target rate: {args.circular_target_rate:.4f}")
    print(f"Circular rings count: {num_circular}")
    print(f"Circular transactions: {len(circular_df)}")
    print()
    print(f"Total augmented transactions: {len(combined)}")
    total_pos = len(peel_df) + len(smurf_df) + len(circular_df)
