"""Peel-chain report for the 25k multi-pattern dataset.

Extracts peel-chain transactions from the 25k augmented dataset,
reconstructs chain topology, prints a detailed terminal report,
and saves visualisation PNGs to ``reports/25k/Peel Chain Visualization/``.
"""
...
def _load_peel_rows(transactions_path: Path, labels_path: Path | None) -> pd.DataFrame:
    ...
    # If the augmented file contains a ``pattern`` column (multi-pattern),
    # filter directly.
    if "pattern" in df.columns:
        return df[df["pattern"].astype(str).str.lower() == "peel_chain"].copy()

    # Fall back to the label file's ``label_note`` column.
    ...
    if "label_note" in labels.columns:
        peel_hashes = set(
            labels.loc[labels["label_note"].astype(str).str.lower() == "peel_chain", "tx_hash"].astype(str)
        )
    else:
...
def _plot_chain_amount_profiles(details: pd.DataFrame, out_path: Path, max_chains: int = 15) -> None:
    ...
    plt.title("25k Dataset — Peel-Chain Amount Decay by Hop (Sampled Chains)")
...
def _plot_linear_chain_layout(details: pd.DataFrame, summary: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.xticks(range(1, max_hop + 1))
    plt.title("25k Dataset — Linear Peel-Chain Layout (One Horizontal Row per Chain)")
...
def _plot_chain_length_distribution(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.title("25k Dataset — Peel-Chain Wallet Length Distribution")
...
def _print_terminal_report(details: pd.DataFrame, summary: pd.DataFrame) -> None:
    print("=" * 80)
    print("PEEL CHAIN REPORT — 25K MULTI-PATTERN DATASET")
    print("=" * 80)
    ...
    print("-" * 80)
    print("CHAIN DETAILS (first 30 chains)")
    print("-" * 80)

    for _, row in summary.sort_values("chain_id").head(30).iterrows():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Peel-chain report for the 25k multi-pattern dataset.")
    parser.add_argument(
        "--transactions",
    ...
    parser.add_argument("--out-dir", default="reports/25k/Peel Chain Visualization")
    ...
    profile_plot = out_dir / "peel_chain_amount_profiles_25k.png"
    layout_plot = out_dir / "peel_chain_linear_layout_25k.png"
    length_plot = out_dir / "peel_chain_length_distribution_25k.png"
